 ...
 *
 * ES6, linted
 * Accept either an element id or a dom element
 * Dynamic pattern passed on significant figures
 *
 * Date: 2019-02-24
 * Version: 1.1.0
 */
...
export class SegmentDisplay {
  ...
  constructor(canvasIdOrElement, major, minor) {
    this.canvasIdOrElement = canvasIdOrElement
    this.major = major
    this.minor = minor
    let s = ''
    while (major > 0) {
  ...
  formatValue(value) {
    ...
    const fmtLen = value.length
    const point = value.indexOf('.')
    let sdist = this.major - point
    let mdist = this.minor - (fmtLen - point - 1)
    while (sdist > 0) {
      value = `0${value}`
      sdist -= 1
    }
  ...
  draw() {
    let display
    if (typeof this.canvasIdOrElement === 'string') {
      display = document.getElementById(this.canvasIdOrElement)
    } else {
